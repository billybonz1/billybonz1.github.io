var people_counter = function(adult,children,max){
	this.adult = adult;
	this.children = children;
	this.max = max;
	this.sum = this.adult + this.children;
	this.count = function(value,action){
		if(action == "up"){
...
var pc = new people_counter(adult,children,12);
$(document).ready(function() {

	$("select.lang").chosen({disable_search_threshold: 10});

	

	$(".pc-arrows").on("click",function(e){
		e.preventDefault();
		var action = $(this).data("action");
		var value = $(this).data("value");
		pc.count(value,action);
		$(".pc-counter[data-value="+value+"]").text(pc[value]);
		$("input[name="+value+"]").val(pc[value]);
	});
